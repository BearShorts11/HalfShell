...
namespace PixelCrushers.DialogueSystem.SequencerCommands
{
    ...
    /// <summary>
    /// Syntax: FMODWait(event, [eventPath], [parameter], [value], [multisound])
    /// 
    /// Plays an FMOD event.
    ...
    /// - value: Optional Value to set parameter to. For none, omit or leave blank.
    /// - multisound: Event contains multiple sounds with pauses.
    /// </summary>
    public class SequencerCommandFMODWait : SequencerCommand
    {
        ...
        public virtual IEnumerator Start()
        {
            ...
            multisound = GetParameterAs<bool>(4, false);

            if (DialogueDebug.logInfo)
            {
                Debug.Log($"Dialogue System: FMODWait({voiceData.key}, path={eventPath}, param={paramName}, value={paramValue}, multisound={multisound})");
            }
        ...
        {
            if (multisound) return voiceData.eventCallbackType != EVENT_CALLBACK_TYPE.DESTROYED;
